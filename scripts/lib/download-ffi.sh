...
find "${FFI_DIR:?}/" -name "*.so" | xargs rm
find "${FFI_DIR:?}/" -name "*.dylib" | xargs rm
find "${FFI_DIR:?}/" -name "*.dll" | xargs rm
find "${FFI_DIR:?}/" -name "*.h" | xargs rm

mkdir -p "$FFI_DIR/osx_aarch64"
mkdir -p "$FFI_DIR/osx_x86_64"
mkdir -p "$FFI_DIR/linux_aarch64"
mkdir -p "$FFI_DIR/linux_x86_64"
mkdir -p "$FFI_DIR/windows_x86_64"
...
if [ -z "${ONLY_DOWNLOAD_PACT_FOR_WINDOWS:-}" ]; then
  download_ffi "linux-x86_64.so.gz" "lib" "linux_x86_64/libpact_ffi.so.gz"
  download_ffi "linux-aarch64.so.gz" "lib" "linux_aarch64/libpact_ffi.so.gz"
  download_ffi "osx-x86_64.dylib.gz" "lib" "osx_x86_64/libpact_ffi.dylib.gz"
  download_ffi "osx-aarch64-apple-darwin.dylib.gz" "lib" "osx_aarch64/libpact_ffi.dylib.gz"
else
  warn "Skipped download of non-windows FFI libs because ONLY_DOWNLOAD_PACT_FOR_WINDOWS is set"
fi

download_ffi "windows-x86_64.dll.gz" "" "windows_x86_64/pact_ffi.dll.gz"
